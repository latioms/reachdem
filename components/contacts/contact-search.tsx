  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
        e.preventDefault()
        const searchInput = document.querySelector('input[data-search="contacts"]') as HTMLInputElement
        if (searchInput) {
          searchInput.focus()
        }
      }
    ...
    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
      <div className="relative flex-1 max-w-md">
        ...
        <Input
          ...
          placeholder={placeholder || dictionary?.contacts?.search?.placeholder || "Rechercher un contact..."}
          value={searchTerm}
      ...
      {searchTerm && resultsCount !== undefined && (
        <div className="text-sm text-muted-foreground whitespace-nowrap">
          {resultsCount} contact{resultsCount !== 1 ? 's' : ''} trouvé{resultsCount !== 1 ? 's' : ''}
