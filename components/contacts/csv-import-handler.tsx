...
const FIELD_MAPPINGS: Record<string, string[]> = {
  'first_name': ['first name', 'firstname', 'prénom', 'prenom', 'given name', 'first', 'fname', 'name'],
  'last_name': ['last name', 'lastname', 'nom', 'nom de famille', 'surname', 'family name', 'last', 'lname'],
  'email': ['email', 'mail', 'e-mail', 'courriel', 'adresse email', 'adresse e-mail', 'email1', 'email2'],
...
export function CsvImportTab({ onSuccess, dictionary }: CsvImportProps) {
  ...
  // Référence pour la fonction de mise à jour de progression
  ...
  const updateProgress = useCallback((processed: number, total: number) => {
    ...
    // Mettre à jour la progression dans le toast si la fonction de mise à jour est disponible
  ...
  const processCsvContent = (content: string) => {
    try {
      ...
      setError(t.processingError || 'Erreur lors de l\'analyse du fichier CSV');
    }
  };
  ...
  const handleImport = async () => {
    ...
    const importFunction = () => {
      return new Promise(async (resolve, reject) => {
        try {
          // Configurer la fonction de mise à jour de progression
          progressRef.current = (progress) => {
            toast.loading(
          ...
          if (validData.length === 0) {
            reject(new Error(t.requiredFields || 'Email est obligatoire'));
            return;
          }
          ...
          for (let i = 0; i < validData.length; i += BATCH_SIZE) {
            ...
            updateProgress(processed, validData.length);
          }
          ...
          // Si tout s'est bien passé, résoudre la promesse
          if (failed === 0) {
            resolve({
              message: `${successful} ${t.successDescription || "contacts importés avec succès"}`,
              result
            });
            ...
            // S'il y a des échecs, considérer comme une réussite partielle
            if (successful > 0) {
              resolve({
                message: `${successful} ${t.successDescription || "contacts importés avec succès"}, ${failed} ${t.failed || "en échec"}`,
                result
              });
            } else {
              // Si aucun contact n'a été importé avec succès, c'est un échec
              reject(new Error(`${failed} ${t.failed || "contacts en échec"}`));
            }
          }
        } catch (err: any) {
          console.error('Error importing contacts:', err);
          reject(err || new Error('Erreur lors de l\'importation des contacts'));
        } finally {
          // Nettoyer la référence de mise à jour de progression
    ...
    toast.promise(importFunction(), {
      loading: <ProgressToast progress={0} />,
